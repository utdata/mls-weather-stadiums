...
hourly_to_daily <- function(dataframe){
  ...
  dataframe_clean_station <- dataframe_clean|> mutate(station = dataframe$STATION[1]) |> mutate(station = as.character(station))
...
new_column_schedules_cleaner <- function(df, years_df){
  ...
  "new_schedule_{year_in_row}_clean" := if_else(is.na(glue_data(df$"new_schedule_{year_in_row}")),glue_data(df$"new_fallback_date_{year_in_row}"),as.Date(glue_data(df$"new_schedule_{year_in_row}"))),
                                               ,
  ...
  "old_schedule_{year_in_row}_clean" := if_else(is.na(glue_data(df$"old_schedule_{year_in_row}")),glue_data(df$"old_fallback_date_{year_in_row}"),as.Date(glue_data(df$"old_schedule_{year_in_row}"))))
  }
  return(df)
  }
...
exact_2025_weather_spring_sched <- weather_data |> mutate(true_date = as.Date(datetime)) |>  filter(true_date %in% spring_fall_2025_schedule)

# 2024-25 Summer Schedule
exact_2025_weather_summer_sched <- weather_data |> mutate(true_date = as.Date(datetime)) |>  filter(true_date %in% summer_spring_2025_schedule)
```
